...
public class Main {
    ...
    // 🔥 Método auxiliar para obtener la factory correcta
    private static LoggingFactory getFactory(LogLevel level) {
        switch (level) {
            case ERROR:
                return ErrorLogFactory.getInstance();
            case INFO:
                return InfoLogFactory.getInstance();
            case DEBUG:
                return DebugLogFactory.getInstance();
            case WARNING:
                return WarningLogFactory.getInstance();
            default:
                ...
                switch (option) {
                    ...
                    case 3:
                        ...
                        try {
                            ...
                            LoggingFactory factory = getFactory(level);
                            Log log = factory.createLog(msg);
                    ...
                    case 4:
                        ...
                        for (int i = 0; i < n; i++) {
                            ...
                            try {
                                LogLevel level = LogLevel.valueOf(t);
                                LoggingFactory factory = getFactory(level);
                                Log log = factory.createLog(m);
